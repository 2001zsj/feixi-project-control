...
网页会自动执行匿名登录，用户不需要注册、输入账号或密码；安全规则只要求存在有效匿名会话。拿到网址的人仍可共同维护，因此网址不要公开传播。
...
- 初始化成功后，云端成为共享主数据；
- localStorage 继续保留为本机缓存和应急副本；读取云端前如发现本机状态不同，系统会额外保留一份 `before-cloud-load` 回退副本。
...
- 每个站点独立存储，76站不是一个大文档；
- 不同站点可并行修改；
- 同一站点使用 revision 乐观锁防止静默覆盖；
- 页面每 60 秒只先检查 `feixi_meta/main` 的变更序号；确认有站点变化时才继续读取站点 revision，减少数据库读取；
- 回到页面或重新获得焦点时也会检查云端更新。
